...
VLLM_URL = "https://llama3gpu.neuraldeep.tech/"
...
class ResponseGenerator(Agent):
    def process(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (rag_response and 'response' in rag_response) or sql_response:
            result["rag_result"] = rag_response['response']
            if sql_response:
                result['sql_result'] = sql_response
            else:
                result['sql_result'] = "Извините, не удалось получить ответ."
        else:
            result["rag_result"] = "Извините, не удалось получить ответ."
...
supervisor = Supervisor([router_agent, rag_agent, sql_agent, response_generator])
